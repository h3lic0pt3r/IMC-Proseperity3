...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # --- Retrieve mid-price and sugar price ---
        macaron_mid = self.get_mid_price(macaron, state.order_depths.get(macaron, None))[0]
        sugar_price = state.observations.conversionObservations[macaron].sugarPrice

        # logger.print(f"[RUN] Timestamp: {state.timestamp}")
        # logger.print(f"[DEBUG] Retrieved mid-price for {macaron}: {macaron_mid}")
        # logger.print(f"[DEBUG] Retrieved sugar price: {sugar_price}")

        # --- Update histories if valid ---
        if macaron_mid is not None and sugar_price is not None:
            self.product_params['price_history'][macaron].append(macaron_mid)
            self.product_params['sugar_price_history'].append(sugar_price)

            # logger.print(f"[DEBUG] Updated {macaron} price history. Length: {len(self.product_params['price_history'][macaron])}")
            # logger.print(f"[DEBUG] Updated sugar price history. Length: {len(self.product_params['sugar_price_history'])}")
        else:
            pass
            # logger.print("[DEBUG] Either macaron mid-price or sugar price is None. Skipping history update.")

        # --- Calculate and log correlation ---
        corr = self.get_macaron_sugar_correlation()
        if corr is not None:
            pass
            # logger.print(f"[DEBUG] Correlation between {macaron} and sugar: {corr:.4f}")
        else:
            pass
            # logger.print("[DEBUG] Correlation could not be calculated (insufficient or mismatched data).")

        # --- Apply strategy and collect orders ---
        macaron_orders = self.correlation_with_sugar_strategy(state)
        if isinstance(macaron_orders, dict):  # Check if order_dict is a dictionary
                for product, orders in macaron_orders.items():
                    if len(orders) > 0:
                        logger.print(f"[CORR] Orders for {product}: {orders}")
                        result[product] = orders
        elif isinstance(macaron_orders, list):  # Check if order_dict is a list
            result['MAGNIFICENT_MACARONS'] = macaron_orders# Direct assignment, as it's already a list
    ...
    def get_macaron_sugar_correlation(self, lookback=50):
        mac_history = list(self.product_params['price_history']['MAGNIFICENT_MACARONS'])[-lookback:]
    ...
    def correlation_with_sugar_strategy(self, state):
        ...
        max_pos = self.product_params['max_position'][follower]
        lookback = 60
        max_lag = 40
        corr_threshold = 0.5
        ...
        macaron_history = self.product_params['price_history'][follower]
        ...
        if sunlight_index is not None and sunlight_index < sunlight_threshold:
            # logger.print("[DEBUG] Sunlight index is low. Using omega_market_maker strategy.")
    ...
    def omega_market_maker_sugar(self, product, order_depth, current_position, timestamp):
        ...
        # momentum = self.calculate_momentum(list(self.product_params['price_history'][product]))
        # if momentum < -self.product_params['momentum_threshold']:
    ...
    def omega_market_maker(self, product, order_depth, current_position, timestamp):
        ...
        # 3. **Aggressive Downward Trend Reaction**: Use rate of change (momentum) to detect downward trends more quickly
        # momentum = self.calculate_momentum(list(self.product_params['price_history'][product]))
        # if momentum < -self.product_params['momentum_threshold']:
        #     # If the price is trending downward aggressively, tighten the spread and take larger positions
        #     self.product_params['gamma'][product] = min(self.product_params['gamma'][product] * 1.5, 2.0)  # Increase gamma to respond faster
        #     dynamic_spread *= 0.5  # Tighten the spread to capture the smaller moves
    ...
    def calculate_correlation(self, product1: str, product2: str, window: int = 10) -> float:
        ...
        prices1 = list(self.product_params['price_history'][product1])[-window:]
        prices2 = list(self.product_params['price_history'][product2])[-window:]
    ...
    def trip_strategy(self, product1, product2,product3, state : TradingState):
        ...
        for produ in prods:
            if len(self.product_params['price_history'][produ]) ==0:
                return order_dict
...
# def 
    ...
    def get_mid_price(self, product, order_depth, window_size=3):
        ...
        # if strategy == 'mid':
        # logger.print(self.product_params['price_history'][product])
        self.product_params['price_history'][product].append(mid_price)
